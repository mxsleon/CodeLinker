...
async def update_user_self_password(
    ...
    user_result = await query_sql_with_params(
        pool, sql=query_self_info_sql, params=None
    )

    if user_result is None:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="账户已更新，请重新登录"
        )
    ...
    if new_user_name:
        ...
        count_num = await execute_sql_with_params(
            pool, sql=sql_count_new_user_name, fetch=True
        )
    ...
    # 拼接sql
    update_user_sql = update_self_management_sql(
        id=cur_user_id, new_username=new_user_name, new_password=new_password_hash
    )

    # 执行sql
    data_update_user_sql = await execute_sql_with_params(
        pool, sql=update_user_sql, fetch=False
    )

    # 已经修改成功，并返回影响行数 1
    if int(data_update_user_sql) == 1:
        if new_user_name:
            sql_get_self_info = get_user_info_sql_other(
                username=new_user_name, role=cur_user_role
            )
        else:
            sql_get_self_info = get_user_info_sql_other(
                username=cur_user_name, role=cur_user_role
            )
