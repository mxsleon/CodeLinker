...
from db.jinja2_sql_user import (
    insert_into_new_user,
    get_user_info_sql,
    get_user_info_sql_all,
    get_user_info_sql_other,
    update_user_info_sql,
    update_user_forget_password_sql,
)
...
router = APIRouter(
    ...
    responses={
        404: {"description": "未找到此接口"},
        500: {"description": "服务其内部错误"},
    },  # 全局响应
)
...
- 响应体**UserResponse**
    ...
    responses={
        ...
        400: {
            "description": "用户名已存在或密码不符合要求",
            "content": {"application/json": {"example": {"detail": "string"}}},
        },
        403: {
            "description": "当前身份无权限创建此级用户",
            "content": {"application/json": {"example": {"detail": "string"}}},
        },
        422: {
...
async def create_user(
    ...
    create_user_is_admin = (
        1 if user_data.role in [RoleEnum.ADMIN, RoleEnum.SUPER_ADMIN] else 0
    )
    ...
    if RoleEnum(current_user_role).weight <= create_user_role.weight:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="当前身份无权限创建此级用户"
        )

    # 1. 检查用户名是否唯一
    user_name_from_db_sql = get_sql_query_user(create_user_name)
    user_name_from_db = await query_sql_with_params(
        pool, sql=user_name_from_db_sql, params=None
    )
    ...
    # 6. 构建sql插入语句
    new_user_sql = insert_into_new_user(
    ...
    sql_response_num = await execute_sql_with_params(
        pool, sql=new_user_sql, params=None, fetch=False
    )
    if sql_response_num < 1:
        return JSONResponse(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            content={"detail": f"用户创建失败，未知原因，请重试"},
        )
    else:

        sql_get_new_user = get_sql_query_user(username=create_user_name)
        sql_response_new_user = await execute_sql_with_params(
            pool=pool, sql=sql_get_new_user, params=None, fetch=True
        )
...
async def get_user_info(
    query: Literal["self", "all", "other"] = Query(
        default="self",
        title="查询的用户，默认为自身",
        description="查询的类型，为空时直接返回自身信息",
    ),
    query_type: Literal["exact", "fuzzy"] = Query(
        default="exact", title="查询类型", description="查询的类型，默认为精确查找"
    ),
    username: str = Query(
        default=None,
        title="查询用户名",
        description="查询的用户名，默认为空，当为模糊查询时，则查询所有用户名中带有username的用户",
    ),
    pool: Pool = Depends(get_db_pool),
    current_user: dict = Depends(get_current_user),
):  # JWT 验证依赖):
    ...
    if query == "self":
        ...
        data_self_info = await execute_sql_with_params(
            pool=pool, sql=sql_get_self, params=None, fetch=True
        )
        # 将查询结果转换为模型实例列表
        validated_users = [UserResponse(**user_data) for user_data in data_self_info]
        return validated_users

    # 当查询类型不为自身时，通过权重判断，为普通用户返回403
    if role_num <= 1:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="当前身份无权限查询"
        )
    else:
        if query == "all":
            sql_get_all = get_user_info_sql_all(role=role_enum)
            dict_data_all_info = await execute_sql_with_params(
                pool=pool, sql=sql_get_all, params=None, fetch=True
            )
            # 将查询结果转换为模型实例列表
            validated_users = [
                UserResponse(**user_data) for user_data in dict_data_all_info
            ]
            return validated_users

        if query == "other":
            sql_get_other = get_user_info_sql_other(
                username=username, role=role_enum, query_type=query_type
            )
            dict_data_other_info = await query_sql(pool=pool, sql=sql_get_other)
            # 将查询结果转换为模型实例列表
            validated_users = [
                UserResponse(**user_data) for user_data in dict_data_other_info
            ]
            return validated_users
...
async def soft_delete_user(
    delete_type: Literal["soft"] = Query(
        "soft", title="删除类型", description="固定为软删除"
    ),
    ...
    if role_num <= 1:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="当前身份无权限"
        )
    ...
    if len(list_data_user_info) == 0:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="没有找到相关用户"
        )
    ...
    delete_sql = update_user_info_sql(
        username=username, id=user_id, is_active=StatusEnum.INACTIVE
    )
    result = await execute_sql_with_params(
        pool=pool, sql=delete_sql, params=None, fetch=False
    )
    if result == 1:
        sql_get_other = get_user_info_sql_other(username=username, role=role_enum)
        list_data_user_info = await query_sql(pool=pool, sql=sql_get_other)
...
async def update_user_info(
    ...
    new_role: Literal["用户", "管理员"] = Query(
        None, title="角色更新", description="要设置的新角色，非超管请勿传参"
    ),
    activate: Optional[bool] = Query(
        True, title="激活状态", description="设为True激活用户，False则不改变用户状态"
    ),
    clean_locked: Optional[bool] = Query(
        None, title="清除锁定", description="设为True清除登录失败标记"
    ),
    pool: Pool = Depends(get_db_pool),
    current_user: dict = Depends(get_current_user),
):
    ...
    # 防止操作自己
    if user_id == cur_user.user_id and cur_role_enum is not RoleEnum.SUPER_ADMIN:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="传入的参数无效"
        )
    ...
    result = await execute_sql_with_params(
        pool=pool, sql=sql_put_user_info, params=None, fetch=True
    )
    if not result:
    ...
    if clean_locked is None or clean_locked is False:
        ...
        clean_locked = True
    # 进行sql拼接
    update_sql = update_user_info_sql(
        id=user_id,
        username=username,
        role=role,
        is_active=is_active,
    ...
    user_update_sql = await execute_sql_with_params(
        pool=pool, sql=update_sql, params=None, fetch=False
    )
    ...
    if user_update_sql == 1:
        ...
        raise HTTPException(
            status_code=status.HTTP_501_NOT_IMPLEMENTED, detail="未知原因失败，请重试"
        )
...
async def update_user_forget_password(
    ...
    username: str = Query(..., title="用户名", description="要更新的用户名"),
    clean_locked: Optional[bool] = Query(
        True, title="清除锁定", description="设为True清除登录失败标记"
    ),
    pool: Pool = Depends(get_db_pool),
    current_user: dict = Depends(get_current_user),
):
    ...
    # 进行sql拼接
    update_sql = update_user_forget_password_sql(
        id=user_id, username=username, clean_locked=clean_locked
    )
    user_update_sql = await execute_sql_with_params(
        pool=pool, sql=update_sql, params=None, fetch=False
    )
