...
@router.post("/login", response_model=Token, summary="用户登录认证",description="""
## 详细描述
...
""",
          responses={
              ...
              403: {"description": "账户已被禁用",        "content": {"application/json": {"example": {"detail": "账户已被禁用"}}}},
              422: {"description": "参数校验失败",        "content":{"application/json": {"example": {"detail": "参数校验失败"}}}},
              423: {"description": "账户已锁定",         "content":{"application/json": {"example": {"detail": "账户已锁定"} }}},
          })
...
async def login_for_access_token(
    ...
    # if not password == user_result['password_hash']:
        ...
        user_verify_password = await execute_sql_with_params(pool, sql=sql_verify_password,fetch=True)
    ...
    user_verify_password = await execute_sql_with_params(pool, sql=sql_verify_password,
                                                         fetch=True)
